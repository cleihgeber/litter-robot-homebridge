...
function computeHkdf(ikm: Buffer, salt: Buffer): Buffer {
  ...
  const hmac = crypto.createHmac('sha256', prk).update(infoBitsBuffer).digest();
...
export class CognitoSRP {
  ...
  private computePasswordHash(userPoolName: string, username: string, password: string): Buffer {
